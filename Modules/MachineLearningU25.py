...
def MachineLearning(dt):
    ...
    X_lately = read_excel('DatasetU25.xls', sheet_name='ML', header=0, 
    ...
    plt.legend(loc=2)
    plt.title('Linear Regression for the U25 unemployment rates of active population in Greece', fontsize=12)
    plt.xlabel('Date')
    ...
    result['Forecasts'].rename('Forecasted Values').plot(color='red', legend=True)
    plt.legend(loc=2)
    plt.title('Lasso for the U25 unemployment rates of active population in Greece', fontsize=12)
    plt.xlabel('Date')
    plt.ylabel('Unemployment Rate')
    plt.show()
