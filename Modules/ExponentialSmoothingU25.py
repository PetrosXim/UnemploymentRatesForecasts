...
def HoltWinterMethod(dt):
    ...
    series.dropna(axis=0, how='any', thresh=None, subset=None, inplace=True)
    fitted_values1 = HW_Forecasting('Greece',series,'original','add',8)
    fitted_values1 = pd.DataFrame(fitted_values1)
    fitted_values1.columns = ['Greece']
    HW_Forecasting('Greece',series,'original','mul',8)
    
    # Portugal
    series = dt[['Portugal']].copy()
    series.dropna(axis=0, how='any', thresh=None, subset=None, inplace=True)
    fitted_values2 = HW_Forecasting('Portugal',series,'original','mul',7)
    ...
    HW_Forecasting('Portugal',series,'original','add',7)
    
    # Netherlands
    series = dt[['Netherlands']].copy()
    series.dropna(axis=0, how='any', thresh=None, subset=None, inplace=True)
    fitted_values1 = HW_Forecasting('Netherlands',series,'original','mul',7)
    fitted_values1 = pd.DataFrame(fitted_values1)
    fitted_values1.columns = ['Netherlands']
    fitted_values = pd.concat([fitted_values, fitted_values1], axis=1)
    HW_Forecasting('Netherlands',series,'original','add',7)
    
    # Latvia
    series = dt[['Latvia']].copy()
    series.dropna(axis=0, how='any', thresh=None, subset=None, inplace=True)
    fitted_values1 = HW_Forecasting('Latvia',series,'original','mul',7)  
    fitted_values1 = pd.DataFrame(fitted_values1)
    fitted_values1.columns = ['Latvia']
    fitted_values = pd.concat([fitted_values, fitted_values1], axis=1)
    HW_Forecasting('Latvia',series,'original','add',7) 
    
    
    # Poland
    series = dt[['Poland']].copy()
    series.dropna(axis=0, how='any', thresh=None, subset=None, inplace=True)
    fitted_values1 = HW_Forecasting('Poland',series,'original','mul',7)  
    fitted_values1 = pd.DataFrame(fitted_values1)
    fitted_values1.columns = ['Poland']
    fitted_values = pd.concat([fitted_values, fitted_values1], axis=1)
    HW_Forecasting('Poland',series,'original','add',7)
...
def KPSS_Test(dt):
    ...
    for j in range(len(dt.columns)):
        ...
        print("\n\nKPSS Test for {0}:".format(names[j]))
