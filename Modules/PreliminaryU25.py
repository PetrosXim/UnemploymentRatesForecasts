...
def ReadData():
    dt = read_excel('DatasetU25.xls', sheet_name='Data1', header=0, 
              index_col=0, parse_dates=True, squeeze=True)  
    dt2 = read_excel('DatasetU25.xls', sheet_name='Data2', header=0, 
              index_col=0, parse_dates=True, squeeze=True) 
    dt3 = read_excel('DatasetU25.xls', sheet_name='Data3', header=0, 
              index_col=0, parse_dates=True, squeeze=True) 
    return dt,dt2,dt3
...
def TimePlot(dt):
    ...
    for i in range(len(dt.columns)):
        ...
        plt.gca().set(title='U25 unemployment rates in {0} from {1} to 2020'.format(names[i],startingpoint[i]), 
...
def TwoSidesPlot(dt):
    ...
    for i in range(len(dt.columns)):
        ...
        plt.title('U25 unemployment rates of {0} from {1} to 2000 (Two Side View)'.format(names[i],startingpoint[i]), fontsize=12)
        plt.xlabel("Dates")
        plt.ylabel("U25 unemployment percentage\n of active population")
...
def SeasonalPlot(dt):
    ...
    for j in range(len(dt.columns)):
        ...
        # Decoration
        plt.gca().set(xlim=(-0.1, 11), ylim=(0, series[names[j]].max() + 2), ylabel='$Unemployment percentage\n of active population$', xlabel='$Month$')
        plt.yticks(fontsize=12, alpha=.7)
        plt.title("Seasonal plot of U25 unemployment rates in {0} from {1} to 2020".format(names[j],startingpoint[j]), fontsize=12)
        plt.show()     
    return
...
def ACF(dt):
    ...
    for j in range(len(dt.columns)):
        ...
        plot_acf(series, title='ACF plot of U25 unemployment rates in {0} - Histogram format'.format(names[j]), lags=100)
        plt.show()
        
        # PACF plot on 50 time lags

        plot_pacf(series, title='PACF plot of U25 unemployment rates in {0} - Histogram format'.format(names[j]), lags=50)
        plt.show()
    return
